    ...
    def begin_brute_crack(self):
        ...
        for length in range(self.lengthbegin, self.lengthend + 1):
            ...
            for passwd in product(gridpoints, repeat=length):
                phash = generator.generate_hash(
                    generator.generate_gesture_string(passwd)
                )
                if phash == self.phash:
                    return passwd
        return False
...
def usage():
    options = [
        ...
        {"short": "-w", "long": "--wordlist", "descr": "Use password wordlist"},
        {
            "short": "-a",
            "long": "--alpha",
            "descr": "Include letters in password generation/cracking",
        },
        {
            "short": "-n",
            "long": "--numeric",
            "descr": "Include numbers in password generation/cracking",
        },
        {
            "short": "-b",
            "long": "--symbols",
            "descr": "Include symbols in password generation/cracking",
        },
        {"title": "Gesture Cracking/Generation:"},
        {
            "short": "-g",
            "long": "--gridsize",
            "descr": "Grid square size eg: -g 3 (for 3x3)",
        },
    ]
...
def handle_crack(opts):
    if opts.lock == opts.PIN:
        ...
        if opts.passwd_length != None:
            opts.passwd_length_begin = opts.passwd_length
            opts.passwd_length_end = opts.passwd_length
        cracker = PasswordGestureCracker(
            opts.passwd,
            opts.gridX,
            opts.gridY,
            opts.passwd_length_begin,
